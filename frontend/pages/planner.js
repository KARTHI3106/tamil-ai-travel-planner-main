...
export default function Home() {
  ...
  const [mode, setMode] = useState('bus'); // Transport mode
  ...
  async function handleTextSubmit(e) {
    ...
    setError('');
    try {
      const fullQuery = `[பயண முறை: ${mode}] ${text.trim()}`;
      const data = await sendQuery(fullQuery);
      setResult(data);
  ...
  async function handleVoiceTranscript(transcript) {
    ...
    setError('');
    try {
      const fullQuery = `[பயண முறை: ${mode}] ${transcript}`;
      const data = await sendQuery(fullQuery);
      setResult(data);
      setRecent(prev => [data, ...prev].slice(0, 10));
    } catch (err) {
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="குரல் மற்றும் உரை அடிப்படையில் செயல்படும் தமிழ் பயண திட்டமிடுபவர்" />
      ...
      <div className={elderlyMode ? 'elderly-mode' : ''} style={containerStyle}>
        ...
        <header style={headerStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 12, marginTop: user ? 0 : 20 }}>
            <i className="ri-plane-fill" style={{ fontSize: 32 }}></i>
            <h1 style={{ margin: 0, fontSize: 28 }}>தமிழ் AI பயண திட்டமிடுபவர்</h1>
          </div>
          <p style={{ margin: '6px 0 0', fontSize: 14, opacity: 0.85 }}>
            தமிழ் AI பயண திட்டமிடுபவர் — குரல் மற்றும் உரை அடிப்படை
          </p>
        ...
        <main style={mainStyle}>
          ...
          <section style={sectionStyle}>
            <h2 style={sectionTitleStyle}><i className="ri-chat-voice-line" style={{ marginRight: '8px' }}></i>உங்கள் பயணத்தை திட்டமிடுங்கள்</h2>
            ...
            <form onSubmit={handleTextSubmit} style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                >
                  {loading ? 'செயலாக்கம்…' : <><i className="ri-search-line" style={{ marginRight: '4px' }}></i> தேடு</>}
                </button>
              </div>
            </form>
          ...
          {/* Result */}
          {loading && (
            <div style={{ textAlign: 'center', padding: 32, color: '#666', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              <i className="ri-loader-4-line" style={{ animation: 'spin 1.5s linear infinite', fontSize: 20 }}></i> திட்டம் தயாராகிறது…
            </div>
          )}
          ...
          {recent.length > 0 && (
            <section className="recent-section" style={{ ...sectionStyle, marginTop: 24 }}>
              ...
              >
                <h2 style={{ ...sectionTitleStyle, margin: 0 }}><i className="ri-history-line" style={{ marginRight: '8px' }}></i>சமீபத்திய தேடல்கள்</h2>
                <span style={{ fontSize: 14, color: '#2887ff' }}>
                  <i className={showRecent ? "ri-arrow-up-s-line" : "ri-arrow-down-s-line"} style={{ marginRight: '4px' }}></i>
